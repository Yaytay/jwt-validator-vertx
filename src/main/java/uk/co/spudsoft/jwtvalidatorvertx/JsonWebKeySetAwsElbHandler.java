...
import uk.co.spudsoft.jwtvalidatorvertx.impl.JWKSAwsElbHandlerImpl;
 ...
 * @author jtalbut
 */
public interface JsonWebKeySetAwsElbHandler extends JsonWebKeySetHandler {
   ...
   * 
   * Each JWKs endpoint must use KIDs that are globally unique (within this handler).
   * Typically only one URL should be used, for one AWS region.
   * 
   ...
   * @param keyBaseUrls Static set of URLs that will be used for obtaining JWKs.
   ...
   * 
   */
  static JsonWebKeySetAwsElbHandler create(WebClient webClient, Collection<String> keyBaseUrls, Duration defaultJwkCacheDuration) {
    return new JWKSAwsElbHandlerImpl(webClient, keyBaseUrls, defaultJwkCacheDuration);
  }
